...
function simulateFundNetValue() {
  ...
  funds.forEach(fund => {
    ...
    // 更新最新净值
    db.prepare('UPDATE funds SET latest_net_value = ? WHERE code = ?')
      .run(newNetValue, fund.code);

    // 插入历史净值
    db.prepare('INSERT INTO fund_history (code, date, net_value) VALUES (?, ?, ?)')
      .run(fund.code, formatDate(virtualDate), newNetValue);
  });
...
// 每分钟模拟一次
setInterval(simulateFundNetValue, 600 * 1000);
